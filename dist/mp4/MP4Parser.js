...
export class MP4Parser extends BasicParser {
    static read_BE_Integer(array, signed) {
        const integerType = (signed ? "INT" : "UINT") +
            array.length * 8 +
            (array.length > 1 ? "_BE" : "");
    ...
    async parse() {
        ...
        const audioTracks = this.tracks.filter((track) => {
            return (track.soundSampleDescription.length >= 1 &&
        ...
        return metaAtom.readAtoms(this.tokenizer, async (child, remaining) => {
            ...
            switch (child.header.name) {
                ...
                default:
                    ...
                    this.addWarning("Unsupported meta-item: " +
                        tagKey +
                        "[" +
                        child.header.name +
                        "] => value=" +
    ...
    async parseChapterTrack(chapterTrack, track, len) {
        if (!chapterTrack.sampleSize) {
            if (chapterTrack.chunkOffsetTable.length !==
        ...
        for (let i = 0; i < chapterTrack.chunkOffsetTable.length && len > 0; ++i) {
            ...
            const sampleSize = chapterTrack.sampleSize > 0
    ...
    getSamplesPerChunk(chunkId, stcTable) {
        for (let i = 0; i < stcTable.length - 1; ++i) {
            if (chunkId >= stcTable[i].firstChunk &&
